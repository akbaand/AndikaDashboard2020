...
app.layout = html.Div([
    ...
    html.H3('by Andika Akbar Hermawan'),
...
def render_content(tab):
    if tab == 'tab-1':
        ...
        id='dropdown',
        options=[
            {'label': 'Filter Methods of KMeans', 'value': 12},
            {'label': 'Wrapper Methods with RFE', 'value': 13},
            {'label': 'Ensemble Methods', 'value': 14},
        ], 
        value=12
        ),
        html.Div(id='FS_png'),
...
def Exporatory_Data_Analysis(radio_year):
...
@app.callback(Output('FS_png','children'),
              Input('dropdown','value'))
def FeatureSelection_Data_Analysis(dropdown1_year):
    if dropdown1_year == 12:
        return html.Div([
            html.H3('Filter Methods of KMeans'),
            generate_table(df)
            ])
...
def Regression_Data_Analysis(dropdown_year):
